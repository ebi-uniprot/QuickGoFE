...
wsService.factory('informationService', [
    '$http',
    'ENV',
    function ($http, ENV) {
        return {
...
wsService.factory('presetsService', [
    '$http',
    'ENV',
    function ($http, ENV) {
        return {
...
wsService.factory('PreDefinedSlimSets', [
    '$resource',
    'ENV',
...
wsService.factory('PreDefinedSlimSetDetail', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
        return $resource(ENV.apiEndpoint + '/predefinedSetTerms/:setId', {
            setId: '@id'
        }, {
            query: {
                method: 'GET',
    ...
    function ($http, ENV) {
        return {
            ...
            getAllStats: function (termId, limit) {
                return $http.get(ENV.apiEndpoint + '/annotation/coterms/' + termId + (limit
                    ? '?limit=' + limit
                    : ''));
            },
            getManualStats: function (termId, limit) {
                return $http.get(ENV.apiEndpoint + '/annotation/coterms/' + termId + '?source=MANUAL' + (limit
                    ? '&limit=' + limit
                    : ''));
            },
            ...
            var taxaInfoWithRedirectedId = _.map(taxaInfo, function (d) {
                if (redirectionMap[d.id]) {
                    var updatedId = redirectionMap[d.id]
                        .redirectLocation
                        .substring(redirectionMap[d.id].redirectLocation.lastIndexOf('/') + 1);
                    $rootScope
                        .alerts
                        .push({
                            type: 'warning',
...
wsService.factory('downloadService', [
    '$http',
    'ENV',
    function ($http, ENV) {
        return {
...
wsService.factory('geneProductService', [
    '$http',
    'ENV',
    function ($http, ENV) {
        return {
...
wsService.factory('searchService', [
    '$http',
    'ENV',
    function ($http, ENV) {
    ...
    function ($http, $location) {
        return {
            getDbXrefs: function () {
                ...
                return $http.get('http://snapshot.geneontology.org/metadata/db-xrefs.json', {cache: true});
            },
            ...
            getLinkforId: function (name, id, xrefs) {
                ...
                if (name === 'GO') {
                    return $location
                        .absUrl()
                        .replace($location.path(), '/term/GO:' + id);
                } else if (name === 'TAXON') {
...
wsService.factory('chartService', [
    '$http',
    'ENV',
    function ($http, ENV) {
        return {
...
wsService.factory('annotationUpdates', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
...
wsService.factory('goTermHistory', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
        return $resource(ENV.apiEndpoint + '/dataset/goTermHistory', {
            from: '@from',
            to: '@to',
            limit: '@limit'
        }, {
...
wsService.factory('taxonConstraints', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
...
wsService.factory('annotationPostProRules', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
...
wsService.factory('annotationBlacklist', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
...
wsService.factory('ontologies', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
        return $resource(ENV.apiEndpoint + '/terms/:ontology', {
            ontology: '@ontology'
        }, {
            query: {
                method: 'GET',
...
wsService.factory('limitChecker', [
    ...
    function (hardCodedDataService, filterService, $rootScope) {
        return {
            addAboveLimitError: function (uploadLimit) {
                $rootScope
                    .alerts
                    .push(hardCodedDataService.getTermsLimitMsg(uploadLimit));
            },
            ...
            getMergedItems: function (dest, items, limit) {
                ...
                if (this.isOverLimit(merged, limit)) {
                    $rootScope
                        .alerts
                        .push(hardCodedDataService.getTermsLimitMsg(limit));
                    return dest;
                } else {
                    return merged;
                }
...
//@deprecated
wsService.factory('searchfull', [
    '$resource',
    'ENV',
    function ($resource, ENV) {
        return $resource(ENV.apiEndpoint + '/searchfull', {
            text: '@text',
            format: 'JSON',
            page: '@page',
            row: '@rows',
