...
validationModule.factory('validationService', function() {
  ...
  validationService.validateGeneProduct = function(id) {
    var matches = (id.match(/^(\w+:)?([OPQ][0-9][A-Z0-9]{3}[0-9]|[A-NR-Z]([0-9][A-Z][A-Z0-9]{2}){1,2}[0-9])((-[0-9]+)|:PRO_[0-9]{10}|:VAR_[0-9]{6}){0,1}$/) || id.match(/^(\w+:)?EBI-[0-9]+$/) || id.match(/^(\w+:)?URS[0-9A-F]{10}(_[0-9]+){0,1}$/));
    return matches;
  };
